...
contract InterAccountTransfer is BaseReentrancyGuard {
    ...
    constructor(address ownerAddr) {
        if (ownerAddr == address(0)) revert AddressRequired();
        owner = ownerAddr;
    }
    ...
    function addToWhitelist (address addr) external nonReentrant onlyOwner {
        if (addr == address(0)) revert AddressRequired();
        isWhitelisted[addr] = true;
        emit OnAddressEnabled(addr);
    }

    function removeFromWhitelist (address addr) external nonReentrant onlyOwner {
        if (addr == address(0)) revert AddressRequired();
    ...
    function interAccountTransfer (
        ...
        if (!isWhitelisted[destinationAccount]) revert NotWhitelisted();

        SafeERC20.safeTransferFrom(token, msg.sender, destinationAccount, amount);
    }
}