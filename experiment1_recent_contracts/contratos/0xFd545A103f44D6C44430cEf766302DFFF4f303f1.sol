// SPDX-License-Identifier: MIT
  ...
  /*
    Website: https://Shihtzu.exchange
    X/Twitter: https://x.com/Shihtzuexchange
    Telegram: https://t.me/Shihtzuexchange

    Shihtzu Exchange Universe is the first Metaverse-based meme coin for 1000X Growth
  */
...
contract ShihtzuExchange is ERC20, Ownable {
    ...
    constructor() ERC20("Shihtzu Exchange", "SHIHTZU") payable {
    ...
    function _transfer(
        ...
        require(uniV2Pair != address(0) || excluded, "SHIHTZU: Liquidity pair not yet created.");
    ...
    function updateFeeThreshold(uint256 newThreshold) external {
        ...
        require(newThreshold >= totalSupply().mul(1).div(100000), "SHIHTZU: Swap threshold cannot be lower than 0.001% total supply.");
        require(newThreshold <= totalSupply().mul(2).div(100), "SHIHTZU: Swap threshold cannot be higher than 2% total supply.");
        feeThresholdSize = newThreshold;
    }

    function updateFees(uint256 newBuyFee, uint256 newSellFee) external onlyOwner {
        require(newBuyFee <= 25 && newSellFee <= 25, 'SHIHTZU: Attempting to set fee higher than initial fee.');
        swapTax = newBuyFee;
